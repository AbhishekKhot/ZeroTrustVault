...
export class VaultService {
     ...
     */
    async list({ userId, limit = DEFAULT_PAGE_LIMIT, after }) {
     ...
     */
    async create({ userId, iv, encrypted_data }) {
     ...
     */
    async update({ userId, id, iv, encrypted_data }) {
        const updates = {
            updated_at: () => "now()",
        };
        if (iv !== undefined)
